...
const ExclusivityModal = ({ onUnlock }) => {
  ...
  // helpers for styling
  const btnBase    = `flex w-full items-center justify-start font-mono text-xs uppercase font-bold py-3 sm:py-4 px-4 rounded-sm border-[0.5px] transition-colors duration-300`;
  const btnIdle    = `${(username === '' || password === '') ? 'bg-white text-black border-[0.5px] border-black' : 'bg-black disabled:bg-black/70 text-white border-[0.5px] border-black'}`;
      ...
      >
        <div class="flex flex-col space-y-2 w-full">
          ...
          <input
            ...
            className="w-full border-[0.5px] border-black px-4 py-3 sm:py-4 rounded-sm bg-white focus:outline-none font-mono text-xs h-auto uppercase animate-flip-up"
          />
